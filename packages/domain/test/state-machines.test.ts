...
describe("wedding brief", () => {
  it("rejects inverted budgets", () => {
    expect(
      WeddingBrief.safeParse({
        weddingDate: "2027-02-06",
        area: "Lekki",
        guestCount: 250,
        budgetMinKobo: 2_000_000_00,
        budgetMaxKobo: 1_000_000_00,
        priorities: ["venue"],
      }).success,
    ).toBe(false);
  });
...
describe("transaction input boundaries", () => {
  it("rejects malformed payment input and exposes a stable safe error contract", () => {
    expect(
      InitializePaymentInput.safeParse({ bookingId: "not-a-uuid", quoteId: "also-bad" }).success,
    ).toBe(false);
    expect(
      ApiError.safeParse({
        ok: false,
        error: {
          ...
          message: safeApiMessage("INVALID_REQUEST"),
  ...
  it("requires a stable idempotency key and meaningful request", () => {
    expect(
      BookingRequestInput.safeParse({
        vendorId: "10000000-0000-4000-8000-000000000001",
        packageId: null,
        weddingBriefId: "20000000-0000-4000-8000-000000000001",
        clientRequestId: "30000000-0000-4000-8000-000000000001",
        guestCount: 150,
        requirements: "Accessible arrival and backup power are required.",
      }).success,
    ).toBe(true);
    expect(
      BookingRequestInput.safeParse({
  ...
  it("rejects an unconfirmed or oversized deposit quote", () => {
    const quote = {
      ...
      depositAmountKobo: 12000,
      expiresAt: "2027-01-01T00:00:00.000Z",
      termsVersion: "v1",
      cancellationSummary: "Cancellation consequences are shown before acceptance.",
      inclusions: ["Hall"],
      exclusions: ["Decor"],
      paymentSchedule: "Deposit now",
      availabilityConfirmed: true as const,
    };
    expect(QuoteInput.safeParse(quote).success).toBe(false);
    expect(
      QuoteInput.safeParse({ ...quote, depositAmountKobo: 5000, availabilityConfirmed: false })
        .success,
    ).toBe(false);
  });
});
