...
Deno.serve(async (request) => {
  ...
  if (Deno.env.get("PAYMENTS_SANDBOX_ENABLED") !== "true" && Deno.env.get("PAYMENTS_LIVE_ENABLED") !== "true") return failure("PAYMENT_GATE_CLOSED",503);
  ...
  if (!booking || !quote || booking.customer_id !== user.id || quote.booking_id !== booking.id) return failure("NOT_FOUND",404);
  ...
  if (existing?.authorization_url) return success({authorizationUrl:existing.authorization_url,reference:existing.provider_reference,reused:true});
  ...
  return success({authorizationUrl:result.data.authorization_url,accessCode:result.data.access_code,reference});
});
