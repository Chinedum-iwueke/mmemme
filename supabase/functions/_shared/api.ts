...
export const InitializePaymentRequest=z.object({bookingId:Uuid,quoteId:Uuid}).strict();
export const RefundRequest=z.object({refundId:Uuid}).strict();
export type ErrorCode="INVALID_REQUEST"|"UNAUTHORIZED"|"FORBIDDEN"|"NOT_FOUND"|"CONFLICT"|"FEATURE_DISABLED"|"PAYMENT_GATE_CLOSED"|"PROVIDER_UNAVAILABLE"|"INTERNAL_ERROR";
...
export function failure(code:ErrorCode,status:number,id=correlationId()){return json({ok:false,error:{code,message:messages[code],correlationId:id}},status)}
export function success<T>(data:T,id=correlationId(),status=200){return json({ok:true,data,correlationId:id},status)}
