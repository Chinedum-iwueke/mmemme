...
export default async function OperationsDashboard() {
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand">mmemme <small style={{fontSize:11,opacity:.65}}>OPS</small></div><nav className="nav" aria-label="Operations navigation">
    <a className="active" href="/"><LayoutDashboard size={17}/> Dashboard</a><a href="/bookings"><BookOpenCheck size={17}/> Booking requests</a><a href="/vendors"><Store size={17}/> Vendors</a><a href="/vendors"><ShieldCheck size={17}/> Verification</a><a href="/money"><CircleDollarSign size={17}/> Money & ledger</a><a href="#"><MessageSquareText size={17}/> Support</a>
    ...
    <div className="grid"><section className="panel"><div className="panel-head"><h2>Requests needing attention</h2><a className="link" href="/bookings">View queue</a></div>{requests.map(([name, detail, state])=><div className="request" key={name}><div><strong>{name}</strong><p>{detail}</p></div><span className="pill">{state}</span></div>)}</section>
